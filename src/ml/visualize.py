...
def show_cmx_gif(results: List[Dict[str, float]],
    ...
    for i, img_path in enumerate(cmxs, 1):
        ...
        ax1.axis("off")
        tb = ax1.table(cellText=converted,
                       colLabels=keys,
                       rowLabels=row_idxs,
                       loc="center",
                       bbox=[0, 0, 1, 1])
...
def visualize_all_cmxs(rst: list, cmx_pathes: Iterable[PathLike], ave: dict,
                       dst: PathLike, show_execlude_outlier: bool = False) -> None:
    images = []
    dst = Path(dst)
    # make table
    d = {}
    ...
    if show_execlude_outlier:
        for k in average_widthout_outlier.keys():
            average_widthout_outlier[k] = (sum(average_widthout_outlier[k])
                                           / len(average_widthout_outlier[k]))
...
if __name__ == "__main__":
    ...
    parser_history.add_argument(
        "-t", "--title", help="The title of graph.", default="")
    ...
    parser_cmx = subparsers.add_parser("all_cmx",
                                       help="Show joined result of each trial")
    parser_cmx.add_argument(
        "source", help="The source file\nExpected {'results': List[Dict[str, int | float]]}")
    parser_cmx.add_argument("--dst", "-d", help="Destination of output",
                            default="./output.gif")
    # parser_cmx.add_argument("--use_keys", nargs="+", required=True,
    #                         help="Use key. it show as row titles")
    parser_cmx.add_argument("--cmxs", nargs="+", required=True,
                            help="The paths of use cmx image")
